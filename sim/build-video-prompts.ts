...
/**
 * Which tiers are actually asked for.
 *
 * Only the ambient loops. Tiers 2 to 5 are gods, astras, endings and battle
 * beats, and every one of those subjects now has a painting in the game; a
 * generated clip of a specific figure or weapon is guaranteed to mismatch the
 * art it sits beside, and holding a figure's identity for five seconds is the
 * thing AI video is worst at. Those are animated in code from the art we own.
 *
 * The dropped clips stay in the table below rather than being deleted, as a
 * record of what was considered. Widen this array to bring any of them back.
 */
const GENERATE: Tier[] = [1];
...
const wanted = clips.filter((c) => GENERATE.includes(c.tier));
const out: string[] = [];
...
out.push('');
out.push(`**${wanted.length} clips.** Every prompt carries the same STYLE anchor as the card`);
out.push('art, because motion that does not match the cards reads as glued on.');
out.push('');
out.push('## Six clips, and why only six');
out.push('');
out.push('This file used to ask for 72, across gods, astras, endings and battle beats.');
out.push('It should not have, and the reason is written at the top of its own generator:');
out.push('motion that does not match the cards reads as glued on.');
out.push('');
out.push('Since that was written, every one of those subjects got a PAINTING. 129 cards');
out.push('and 8 deities, all shipped. A generated clip of Agni will not be our Agni, and');
out.push('a generated Brahmastra will not be the Brahmastra the player is holding. It is');
out.push('guaranteed to mismatch the thing it sits next to.');
out.push('');
out.push('Those subjects are also the ones AI video is worst at: a specific figure with');
out.push('an identity to hold for five seconds, rather than weather. So they are built in');
out.push('code instead, out of the paintings we already own. The card art itself is what');
out.push('blooms, shakes and settles when an astra fires, which cannot mismatch the card');
out.push('because it IS the card.');
out.push('');
out.push('What is left is the one thing code cannot fake and video is genuinely good at:');
out.push('drifting atmosphere with no object in it.');
out.push('');
...
out.push('');
out.push('**Sizes: 720p, not 480p.** That advice was written when this file asked for');
out.push('72 clips and the whole set had to fit in roughly 15 MB. It asks for six now,');
out.push('and the art already shipped is 7.6 MB, so there is room. These are full-bleed');
out.push('backgrounds: at 480p upscaled to a phone they go soft, and soft is the one');
out.push('thing a hand-painted look cannot survive. WebM/VP9 first with an MP4/H.264');
out.push('fallback for Safari. Low frame rates are fine and often better; drifting dust');
out.push('at 12-15 fps reads as hand-painted rather than cheap.');
out.push('');
...
out.push('');
out.push('**Order of work.** All six. That is the whole list.');
out.push('');

for (const tier of GENERATE as Tier[]) {
...
const total = wanted.reduce((s, c) => s + Number(c.secs), 0);
out.push('---');
out.push('');
out.push(`**Totals.** ${wanted.length} clips, ${total}s of footage, ` +
  `${wanted.filter((c) => c.loop).length} of them loops.`);
out.push('');

fs.writeFileSync('docs/video-prompts.md', out.join('\n'));
// eslint-disable-next-line no-console
console.log(
  `docs/video-prompts.md written: ${wanted.length} clips, ${total}s total, ` +
    `${wanted.filter((c) => c.loop).length} loops`,
);
